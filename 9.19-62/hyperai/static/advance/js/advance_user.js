...
$.post('/vip/frame_select', {'frame_obj': ''}, function (e) {
    ...
    // $('.form1 .fen-list').eq(0).children().eq(0).find('input').prop('checked',true)
})
        ...
        for (var i = 0,len = public_key.length; i<len;i++){
            if(framework == public_key[i]){
                ...
                for (var i = 0, len = public_img2[framework].length; i < len; i++) {
                    ...
                    var year = new Date(public_img2[framework][i][2]).getFullYear()
                    var month = new Date(public_img2[framework][i][2]).getMonth()+1
                    var date = new Date(public_img2[framework][i][2]).getDate()
                    var hour = new Date(public_img2[framework][i][2]).getHours();
                    var min = new Date(public_img2[framework][i][2]).getMinutes();
                    var second = new Date(public_img2[framework][i][2]).getSeconds();
                    if(month < 10){
                        month = '0' + month
                    }
                    if(date <10){
        ...
        for (var i = 0,len = private_key.length; i<len;i++){
            if(framework == private_key[i]){
                ...
                for (var i = 0, len = private_img2[framework].length; i < len; i++) {
                    ...
                    var year = new Date(private_img2[framework][i][2]).getFullYear()
                    var month = new Date(private_img2[framework][i][2]).getMonth()+1
                    var date = new Date(private_img2[framework][i][2]).getDate()
                    var hour = new Date(private_img2[framework][i][2]).getHours();
                    var min = new Date(private_img2[framework][i][2]).getMinutes();
                    var second = new Date(private_img2[framework][i][2]).getSeconds();
                    if(month < 10){
                        month = '0' + month
                    }
                    if(date <10){
...
function mySubmit_distribute() {
    ...
    // var memory = $('.form2').find('.memory_counts').val()
    // var para_obj ={'ismore':tabs,'node_count':nodes,'gpu_count':gpu,'cpu_count':cpu,'memory_count':memory}
    // $.ajax({
    //     url:'/vip/check_res',
    //     method:'GET',
    //     data:para_obj,
...
$('.single-btn').click(function () {
    ...
    var gpu = $('.form1').find('.gpu_count').val();
    var cpu = $('.form1').find('.cpu_count').val()
    var memory = $('.form1').find('.memory_count').val()
    console.log('---',gpu,cpu)
    var para_obj ={'ismore':tabs,'node_count':nodes,'gpu_count':gpu,'cpu_count':cpu,'memory_count':memory}
    $.ajax({
        url:'/vip/check_res',
        ...
        success:function(e){
            ...
            if (e.a == 0) {
                ...
                var formData = new FormData(document.getElementById("single-form"));
                console.log(formData.get('framework_name'),formData.get('framework_tag'),formData.get('framework_time'))
                $.ajax({
                    type:'POST',
                    url:'/vip/single_create',
                    data:formData,
                    dataType: "json",
                    ...
                    success:function(data){
                        localStorage.clear();
                        localStorage.setItem('data',JSON.stringify({'job_name':$('.vname').val(),'name':data.job_username,'job_id':data.job_id,'statue':data.statue,'create_time':data.create_time,'job_framework':data.job_framework}))
                        window.location.href='/show/advance/monitor'
    ...
    $.ajax({
        ...
        success:function(e){
            ...
            if (e.a == 0) {
                ...
                var formData = new FormData(document.getElementById("distribute-form"));
                $.ajax({
                    type:'POST',
                    url:'/vip/distributed_create',
                    data:formData,
                    dataType: "json",
                    contentType: false, //必须
                    processData: false,
