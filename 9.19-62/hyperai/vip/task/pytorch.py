...
class Pytorch:
    ...
    def create_train_task(self, **kwargs):
        return PytorchTrain(name=self.name,mode=self.mode, **kwargs)


@subclass
class PytorchTrain(TrainTask):

    PYTORCH_LOG = 'au_pytorch_output.log'

    def __init__(self, **kwargs):
        print "pytorch kwargs :" ,kwargs
        super(PytorchTrain, self).__init__(**kwargs)
        self.log_file = self.PYTORCH_LOG
        self.pytorch_log = None
        self.log = None

    def __getstate__(self):
        state = super(PytorchTrain, self).__getstate__()
    ...
    def __setstate__(self, state):
        super(PytorchTrain, self).__setstate__(state)


    @override
    def before_run(self):
        self.log = open(self.path(self.PYTORCH_LOG), 'a+')
    ...
    def after_run(self):
        ...
        super(PytorchTrain, self).after_run()
