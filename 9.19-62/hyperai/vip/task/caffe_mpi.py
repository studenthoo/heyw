...
class CaffeMpi:
    ...
    def create_train_task(self, **kwargs):
        return CaffeTrainMpi(name=self.name, mode=self.mode, **kwargs)


@subclass
class CaffeTrainMpi(TrainTask):
    """
    分布式训练任务
    """
    CAFFE_LOG = 'au_caffempi_output.log'

    def __init__(self, **kwargs):
        print "caffempi kwargs :", kwargs
        super(CaffeTrainMpi, self).__init__(**kwargs)
    ...
    def __getstate__(self):
        state = super(CaffeTrainMpi, self).__getstate__()
    ...
    def __setstate__(self, state):
        super(CaffeTrainMpi, self).__setstate__(state)
    ...
    def after_run(self):
        ...
        super(CaffeTrainMpi, self).after_run()
    ...
    def task_arguments(self, resources, env, full_pod_name=None):
        ...
        args = [
                'kubectl exec %s --' % full_pod_name,
                '/usr/local/mpi/bin/mpirun',
                '-np', '%d' % (self.node_count * int(self.gpu_count)),
                '-hostfile', os.path.join(self.job_dir, 'hostfile'),
                '-bind-to', 'none',
                '-map-by', 'slot',
                '-x', 'PYTHONPATH=/mnt/Caffe-MPI.github.io/python/',
                '--allow-run-as-root',
                '-x', 'NCCL_DEBUG=INFO',
                '-x', 'LD_LIBRARY_PATH',
                '/mnt/Caffe-MPI.github.io/build/tools/caffe', 'train',
                "--solver=%s" % file_path,
                ]
    ...
    def get_output_line_parameter(self,line):
        ...
        if match:
            ...
            step = str(step)
            return {"step": step}
        match = re.search(r'(Train|Test) net output #(\d+): (loss) = (\d+.\d+)', message, flags=re.IGNORECASE)
        if match:
            name = match.group(1)
        ...
        match = re.search(r'(Train|Test) net output #(\d+): (accuracy) = (\d+.\d+)', message, flags=re.IGNORECASE)
        if match:
            name = match.group(1)
        ...
        match = re.search(r'(Train|Test) net output #(\d+): (loss_bbox) = (\d+.\d+)', message, flags=re.IGNORECASE)
        if match:
            name = match.group(1)
        ...
        match = re.search(r'(Train|Test) net output #(\d+): (loss_coverage) = (\d+.\d+)', message, flags=re.IGNORECASE)
        if match:
            name = match.group(1)
        ...
        match = re.search(r'(Train|Test) net output #(\d+): (mAP) = (\d+.\d+)', message, flags=re.IGNORECASE)
        if match:
            name = match.group(1)
        ...
        match = re.search(r'(Train|Test) net output #(\d+): (precision) = (\d+.\d+)', message, flags=re.IGNORECASE)
        if match:
            name = match.group(1)
        ...
        match = re.search(r'(Train|Test) net output #(\d+): (recall) = (\d+.\d+)', message, flags=re.IGNORECASE)
        if match:
            name = match.group(1)
