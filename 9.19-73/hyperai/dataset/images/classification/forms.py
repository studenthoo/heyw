...
from hyperai.utils.forms import validate_required_iff, validate_greater_than
...
class ImageClassificationDatasetForm(ImageDatasetForm):
    ...
    folder_train = utils.forms.StringField(
        ...
        tooltip=(u'指出一个文件夹所有图片的子文件夹，每一个子文件夹应该根据所保存的图像的期望标签来命名')
    )
    # floder_desc = utils.forms.StringField(
    #     u'description',
    #     validate_required_iff =validate_required_iff(method='folder'),
    #
    # )
    ...
    pic_num = utils.forms.SelectField(
        'pic_num',
        choices=[
            (2, 'many'),
            (1, 'one'),
