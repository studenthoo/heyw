...
def new(extension_id):
    ...
    rendered_extension = flask.render_template_string(template.decode('utf-8'), **context)

    # print rendered_extension
...
def create(extension_id):
    ...
    job = None
    try:
        if hasattr(extension_form, 'feature_folder') and hasattr(extension_form, 'label_folder'):
            feature_folder_path = extension_form.feature_folder.data,
            label_folder_path = extension_form.label_folder.data,
        else:
            feature_folder_path = ['']
            label_folder_path = ['']
        # create job
        job = GenericDatasetJob(
            ...
            feature_folder_path=feature_folder_path[0],
            label_folder_path=label_folder_path[0],
        )
...
def show(job, related_jobs=None):
    """
    Called from hyperai.dataset.views.show()
    """

    if job.extension_id == 'image-object-detection':
        return flask.render_template('datasets/generic/show2.html', job=job, related_jobs=related_jobs)
    elif job.extension_id == 'image-processing':
        return flask.render_template('datasets/generic/show_processing.html', job=job, related_jobs=related_jobs)
    elif job.extension_id == 'image-segmentation':
        return flask.render_template('datasets/generic/segmentate.html', job=job, related_jobs=related_jobs)
    elif job.extension_id == 'text-classification':
        return flask.render_template('datasets/generic/show-text.html', job=job, related_jobs=related_jobs)
    elif job.extension_id == 'image-sunnybrook':
        return flask.render_template('datasets/generic/sunnybrook.html', job=job, related_jobs=related_jobs)
    else:
