...
from hyperai.utils.image import embed_image_html
...
class InferenceTask(Task):
    ...
    def __init__(self, username,model, images, epoch, layers, resize, **kwargs):
        """
        Arguments:
    ...
    def after_run(self):
        ...
        if self.inference_data_filename is not None:
            ...
            # print "--->>inference/process_output: inference_inputs, inference_outputs, inference_layers==", \
            #     self.inference_inputs, inference_outputs, inference_layers
    ...
    #                 # if resources[gpu_key].remaining >= 1:
    #                 #     # TODO
    #                 #     self.gpu = 1
    #                 #     # {'gpus': (resources['gpus'], self.gpu_count)}
    #                 #     reserved_resources[gpu_key] = (resources[gpu_key], 1)
                ...
                if resources[gpu_key]:
                    for resource in resources[gpu_key]:
                        if resource.remaining() >= 1:
                            ...
                            reserved_resources[gpu_key] = [(resource.identifier, 1)]
                            break
                print "inference/reserved_resources===", reserved_resources
                return reserved_resources
        return None
    ...
    def task_arguments(self, resources, env):
        ...
        args = [sys.executable,
                os.path.join(os.path.dirname(os.path.abspath(hyperai.__file__)), 'tools', 'inference.py'),
                self.image_list_path if self.image_list_path is not None else self.images,
                self.job_dir,
                self.model.id(),
                '--jobs_dir=%s' % (os.path.join(hyperai.config.config_value('jobs_dir'),self.username+'/'+'jobs')).encode('utf-8'),
