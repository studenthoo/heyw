...
import hyperai.dataset.images.generic.test_views
import hyperai.dataset.generic.test_views
import hyperai.test_views
from hyperai import test_utils
...
class BaseViewsTest(hyperai.test_views.BaseViewsTest):
    """
...
class UserModel(Tower):
    ...
    def loss(self):
        ...
        return hyperai.mse_loss(self.inference, y)
"""
...
class BaseViewsTestWithDataset(BaseViewsTestWithAnyDataset,
                               hyperai.dataset.images.generic.test_views.BaseViewsTestWithDataset):
    """
...
class BaseTestCreation(BaseViewsTestWithDataset):
    ...
    def test_clone(self):
        ...
        job1 = hyperai.webapp.scheduler.get_job(job1_id)
        job2 = hyperai.webapp.scheduler.get_job(job2_id)
...
class BaseTestCreatedWithAnyDataset(BaseViewsTestWithModelWithAnyDataset):
    ...
    def test_save(self):
        job = hyperai.webapp.scheduler.get_job(self.model_id)
...
class BaseTestCreatedWithGradientDataExtension(BaseTestCreatedWithAnyDataset,
                                               hyperai.dataset.generic.test_views.BaseViewsTestWithDataset):
    """
...
class UserModel(Tower):
    ...
    @model_property
    def loss(self):
        return hyperai.mse_loss(self.model, self.y)
"""
...
class TestTensorflowCreatedVariableSizeDataset(BaseTestCreatedWithImageProcessingExtension, test_utils.TensorflowMixin):
    ...
    @classmethod
    def setUpClass(cls):
        raise unittest.SkipTest('Variable-size dataset not supported in Tensorflow/hyperai')
