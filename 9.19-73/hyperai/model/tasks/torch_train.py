...
class TorchTrainTask(TrainTask):
    ...
    def before_run(self, args=None):
        super(TorchTrainTask, self).before_run(args=None)
        ...
        # >>>>  2018-4-26  >>>>
        torch_obj = ManagePod(job_id=self.job.id(), logger=self.logger, job_dir=self.job_dir, task_obj=self, output_log=self.log)
        command = ["/usr/bin/python2"]
        args = ['/root/hyperai/hyperai/pid_rand.py',
                '--args=%s' % (" ".join(args)),
                ]
        try:
            new_path = torch_obj.generate_yaml(gpu_count=self.gpu_count,
            ...
            self.pod_obj = torch_obj
            self.yaml_path = new_path
            return {'obj': torch_obj, 'yaml': new_path}
    ...
    def task_arguments(self, resources, env):
        ...
        if 'gpus' in resources:
            pass
            # gpu_count = resources['gpus'][1]   # +++
            # args.append('--gpu_count=%s' % gpu_count)   # +++
            """
            identifiers = []
    ...
    def process_output(self, line):
        ...
        if level in ['error', 'critical']:
            ...
            # return True
            if re.search(r'thr-id', message):
                # print("\n\n{}\n\n".format(message))
                return True
            # print "* * torch_train/process_out: Return E ."
            return "E"
            # print "* * torch_train/process_out: Return E ."
            # return "E"
            ...
            while p.poll() is None:
                for line in utils.nonblocking_readlines(p.stdout):
                    if self.aborted.is_set():
                        p.terminate()
                        raise hyperai.inference.errors.InferenceError(
    ...
    def infer_many_images(self, images, snapshot_epoch=None, gpu=None):
        ...
        try:  # this try...finally clause is used to clean up the temp directory in any case
            ...
            args = [config_value('torch')['executable'],
                    ...
                    'test.lua',
                    '--testMany=yes',
                    '--allPredictions=yes',  # all predictions are grabbed and formatted as required by hyperai
