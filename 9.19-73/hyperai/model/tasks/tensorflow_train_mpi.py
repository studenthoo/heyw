...
@subclass
class TensorflowTrainTaskMpi(TrainTask):
    """
    Trains a tensorflow model
    """

    TENSORFLOW_LOG = 'tensorflow_mpi_output.log'
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(TensorflowTrainTaskMpi, self).__init__(**kwargs)
        ...
        self.flag = "tf-train-mpi"

        # self.node_count = 2

    def __getstate__(self):
        state = super(TensorflowTrainTaskMpi, self).__getstate__()
    ...
    def __setstate__(self, state):
        super(TensorflowTrainTaskMpi, self).__setstate__(state)
    ...
    def task_arguments(self, resources, env):
        file_path = os.path.join(os.path.dirname(os.path.abspath(hyperai.__file__)), 'tools', 'tensorflow_mpi', 'main.py')
        # file_path = os.path.join(os.path.dirname(os.path.abspath(hyperai.__file__)), 'tools', 'tensorflow',
        #                              'main.py')
        ...
        if self.node_count == 1:
            ...
            args = [
                    ...
                    '-np', '%d' % (self.node_count * self.gpu_count),
    ...
    def before_run(self, args=None):
        # print("Before Run.")
        super(TensorflowTrainTaskMpi, self).before_run(args=None)
        ...
        try:
            ...
            # print(kube_cood)
            # kube_cood.container_prepare(job_id=self.job_id)
            if not kube_cood.container_prepare(job_id=self.job_id):
                return False
            self.pod_name = kube_cood.get_full_pod_name()
            return True
    ...
    def run(self, resources, **kwargs):
        ...
        if hasattr(self, 'pod_name'):
            command = 'kubectl exec {pod_name} -- '.format(pod_name=self.pod_name)
                ...
                for line in utils.nonblocking_readlines(self.p.stdout):
                    ...
                    if line:
                        ...
                        if not value:
                            ...
                            # print "* * tf_mpi.py/run: Set Task Status To Error ."
                            self.status = Status.ERROR
                            # print "* * tf_mpi.py/run: Kill The Subprocess ."
                            self.p.terminate()
                            # self.after_run()
                            break
                    else:
    ...
    @override
    def after_run(self):
        super(TensorflowTrainTaskMpi, self).after_run()
    ...
    def infer_one_image(self, image, snapshot_epoch=None, layers=None, gpu=None):
        ...
        args = [sys.executable,
                os.path.join(os.path.dirname(os.path.abspath(hyperai.__file__)), 'tools', 'tensorflow', 'main.py'),
        ...
        try:
            while p.poll() is None:
                for line in utils.nonblocking_readlines(p.stdout):
                    if self.aborted.is_set():
                        p.terminate()
                        raise hyperai.inference.errors.InferenceError('%s classify one task got aborted. error code - %d' % (self.get_framework_id(), p.returncode))  # noqa
            ...
            if p.poll() is None:
                p.terminate()
            error_message = ''
            if type(e) == hyperai.inference.errors.InferenceError:
            ...
            raise hyperai.inference.errors.InferenceError(error_message)
    ...
    def process_test_output(self, line, predictions, test_category):
        ...
        if level in ['error', 'critical']:
            raise hyperai.inference.errors.InferenceError('%s classify %s task failed with error message - %s' % (
            ...
            try:
                while p.poll() is None:
                    for line in utils.nonblocking_readlines(p.stdout):
                        if self.aborted.is_set():
                            p.terminate()
                            raise hyperai.inference.errors.InferenceError('%s classify many task got aborted.'
    ...
    def get_task_stats(self, epoch=-1):
        ...
        if hasattr(self, "hyperai_version"):
            stats.update({"hyperai version": self.hyperai_version})
