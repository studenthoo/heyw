...
from .ration import Deployment_job
from . import utils
...
# from hyperai.inference import InferenceJob
...
class GpuResource(object):
    ...
    class ResourceAllocation(object):
        ...
        def __init__(self, task, value, label):
            ...
            self.value = value
            self.label = label
    def __init__(self, gpu_max, label_list, cpu_max=100, memory_max=100):
        ...
        self.label_dict = {k: [int(v), int(v)] for k, v in self.label_init_dict.items()}
    ...
    def get_usable_label_gpu(self):
        one_dict = {k: v[-1] for k, v in self.label_dict.items()}
        temp_dict = {}
        for k, v in one_dict.items():
            if v < 0:
                ...
                temp_dict[k] = v
        return temp_dict
        # return {k: v[-1] for k, v in self.label_dict.items()}
    ...
    def deallocate(self, task):
        ...
        for i, a in enumerate(self.allocations):
            if id(task) == id(a[0]):
                ...
                task.logger.info(
                    "< System > < Resources Pool: [%s] > Deallocate the resources Successed . GPU=[%d]" % (
                    task.node_label, a[1]))
                return True
        # for i, a in enumerate(self.allocations):
        #     if id(task) == id(a.task):
...
class RationResource(object):

    def __init__(self, gpu_max,label_list, cpu_max=100, memory_max=100):
        """label_list=[(label01, 2), (label02, 2), (label03, 2), ]"""

        # used_gpu = read_gpu_count()
        # if used_gpu:
        #     self._gpu_max = int(total_gpu)-used_gpu
        # else:
        #     self._gpu_max = int(total_gpu)
        self._gpu_max = gpu_max 
        self.gpu_can_use = self._gpu_max
        self.cpu_max = cpu_max
        self.memory_max = memory_max
        self.allocations = []
        self.label_init_dict = {'inference':self._gpu_max}
        self.label_dict = {'inference':self.gpu_can_use}
    ...
    def remaining(self, value=0, label=None):
        """
        Without judgment, all will pass.
        Returns the amount of this resource that is not being used
        """
...
ration_gpu = config_value('ration_gpu')
ration_label = config_value('ration_label')
...
class Scheduler:
    ...
    def __init__(self, gpu_list=None, verbose=False):
        ...
        num = read_gpu_count()
        # num = 0
        self.ration_resources = {
            'gpus': RationResource(gpu_max=ration_gpu, label_list=ration_label)
        }
        self.resources = {
            ...
            'gpus': GpuResource(gpu_max=cons_gpu_max, label_list=resources_label),
            # 'gpus': gpus_init_obj,
        }
        print "label_list=", resources_label
        # self.resources['gpus'].gpu_can_use = cons_gpu_max - int(num)
        self.ration_resources['gpus'].gpu_can_use = ration_gpu - int(num)
        if self.ration_resources['gpus'].gpu_can_use < 0:
            self.ration_resources['gpus'].gpu_can_use = 0
        self.running = False
        ...
        # _s.setFormatter(logging.Formatter("%(asctime)s %(name)s [%(levelname)s]: %(message)s"))
        # self.mylog.addHandler(_s)
        self.mylog.addHandler(_h)
        self.mylog.setLevel(logging.INFO)
        self.mylog.info("======================================================")
        self.mylog.info("Scheduler started!")
        self.status_update = True
        self.deployment_job_list = []

    def deploy(self):
        from hyperai.utils.resource_monitor2 import Monitor_Resource

        all_deployment_job = self.get_deployment_jobs(None, name=False)
        content = self.short_exec_com()
        for deployment_job in all_deployment_job:
            obj_list = self.parse_pod_msg(deployment_job.id(), content)
            if deployment_job.job_status == 'Running' and obj_list and obj_list[2] == 'Running':
                if deployment_job.resources_deployment == '弹性伸缩模式' or deployment_job.resources_deployment == '固定资源模式':
                    monitor = Monitor_Resource(deployment_job)
                    gevent.spawn(monitor.hw_socketio_updater, deployment_job.id(),
                                 deployment_job.resources_deployment)
                    self.deployment_job_list.append(deployment_job)

        def fun_timer():
            global timer
            timer = threading.Timer(60, fun_timer)
            timer.start()
            for job in self.deployment_job_list:
                job.tasks[0].deal_data(timer)
    ...
    def abort_deployment_jobs(self, username):
        all_jobs = self.get_jobs()
        deployment_jobs = [job.id() for job in all_jobs if
                           job.job_type() == 'deployment_job' and job.username == username]
        return deployment_jobs

    def get_deployment_jobs(self, username, name=True):
        all_jobs = self.ration_jobs.get_jobs()
        if name:
            deployment_jobs = [job for job in all_jobs if
                               job.job_type() == 'deployment_job' and job.username == username]
            return deployment_jobs
        else:
            deployment_jobs = [job for job in all_jobs if
                               job.job_type() == 'deployment_job']
            return deployment_jobs
    ...
    def load_past_jobs_scheduler(self):
        ...
        for dir_name in sorted(os.listdir(alljobs_path)):
            if os.path.isdir(os.path.join(alljobs_path, dir_name)):
                ...
                if year.isalnum() and int(year) < 20180903:
                    continue
                ...
                try:
                    ...
                    if isinstance(job, DistributedJob):
                    ...
                    elif isinstance(job, Deployment_job):
                        if job.job_status == 'Running':
                            self.ration_jobs.put(job)
                        ...
                        job.status = Status.ABORT
                    if not isinstance(job, Deployment_job):
        ...
        print('* * LOAD JOB : %.2f' % (time.time() - start_time))
        pass
    ...
    def get_ration_job(self,job_id):
        if job_id is None:
            return None
        if self.ration_jobs.get_by_job_id(job_id):
            return self.ration_jobs.get_by_job_id(job_id)
    ...
    def delete_job(self, job):
        ...
        if job:
            ...
            if self.other_jobs.remove_by_job_id(job_id):
                pass
                # print "* [{job_id}] Del from other_jobs queue".format(job_id=job.id())
            if self.pending_jobs.remove_by_job_id(job_id):
                pass
                # print "* [{job_id}] Del from pending_jobs queue".format(job_id=job.id())
            if self.running_jobs.remove_by_job_id(job_id):
                pass
                # print "* [{job_id}] Del from running_jobs queue".format(job_id=job.id())
        ...
        try:
            ...
            while not self.shutdown.is_set():
                ...
                for job in self.running_jobs.get_jobs():
                    ...
                    for task in job.tasks:
                        if task.status in (Status.DONE,):
                            ...
                            all_done = False
                            job.status = Status.ABORT
                            # self.mylog.info("* < SCH > Job is aborted: %s", str(job.id()))
                            ...
                            # self.mylog.info("Job is error: %s", str(job.id()))
                            task.logger.info("* < SCH > Job is error. ")
                            job_done_list.append(job)
                            break
                        # elif not isinstance(job, (ModelJob, ParaJob, Deployment_job, DistributedJob)) and task.status in (Status.INIT, Status.WAIT):
                        elif task.status in (Status.INIT, Status.WAIT):
                            all_done = False
                            if id(task) == id(job.get_first_task()):
                                job.status = Status.WAIT
                                # print("* < SCH > This Task Which Is The First was adjusted.")
                                continue
                ...
                for job in job_done_list:
                    ...
                    # if not isinstance(job, (DatasetJob, Deployment_job, PretrainedModelJob)):
                    if isinstance(job, (ModelJob, ParaJob, DistributedJob)):
                        for task in job.tasks:
                ...
                if start_index:
                    for i in range(1, start_index + 1):
                        ...
                        def depend_judge(job):
                            if isinstance(job, ModelJob):
                                if job.dataset.status in [Status.ABORT, Status.ERROR]:
                                    job.status = job.dataset.status
                                    job.abort()  # +++
                                    move_to_other_list.append(job)
                                    return False
                                elif job.dataset.status in [Status.WAIT, Status.INIT, Status.RUN]:
                                    return False
                                elif job.dataset.status in [Status.DONE]:
                                    return True
                                else:
                                    raise ValueError("the status of dataset is unrecognized.")
                            else:
                                return True
                        ...
                        if depend_judge(job):
                            ...
                            if task and task.ready_to_queue():
                                ...
                                if requested_resources is None:
                                    ...
                                    if isinstance(job, (ModelJob, DistributedJob, ParaJob)) or self.reserve_resources(task, requested_resources):
                                        gevent.spawn(self.run_task, task, requested_resources)
                    ...
                    for job in move_to_other_list:
                        ...
                        self.other_jobs.put(job)
                        # job.release_res_about_user()
                        # if not isinstance(job, (DatasetJob, Deployment_job, PretrainedModelJob, InferenceJob)):
                        if isinstance(job, (ModelJob, ParaJob, DistributedJob)):
                            for task in job.tasks:
                                register_user.deallocate_resources(job.username, task)
                        self.mylog.info("Job from pending move to other: %s", str(job.id()))
                pass
                if self.ration_jobs.get_jobs():
                    for job in self.ration_jobs.get_jobs():

                        if hasattr(job, 'job_status') and job.job_status == 'Waiting':
                            gevent.spawn(job.tasks[0].run)
                ...
                if not last_saved or time.time() - last_saved > 15:
                    for job in self.get_jobs():
                        if job.status.is_running():
                            ...
                            self.delete_job(job)
                    for job in self.ration_jobs.get_jobs():
                        if job.job_status in ['Wait','Running','Init']:
                            job.save()
    ...
    def get_gpu_deployment(self):
        all_deployment_job = self.get_deployment_jobs(None, name=False)
        gpu_used_by_deploy = 0
        content = self.short_exec_com()
        res = {}
        for deployment_job in all_deployment_job:
            obj_list = self.parse_pod_msg(deployment_job.id(), content)
            if deployment_job.job_status == 'Running' and obj_list and obj_list[2] == 'Running':
                if deployment_job.resources_deployment == '弹性伸缩模式':
                    num = deployment_job.tasks[0].instance_min
                    gpu_used_by_deploy += int(num)
                    username = deployment_job.username
                    res[username] = num
                elif deployment_job.resources_deployment == '固定资源模式':
                    num = deployment_job.tasks[0].replicas
                    gpu_used_by_deploy += int(num)
                    username = deployment_job.username
                    res[username] = num
        # self.resources['gpus'].gpu_can_use -= gpu_used_by_deploy
        path = '/root/hyperai/deploy_gpu.txt'
