...
from hyperai import utils, log  # noqa
from hyperai.inference.errors import InferenceError  # noqa
from hyperai.job import Job  # noqa
from hyperai.utils.lmdbreader import DbReader  # noqa
from hyperai.config import config_value
# Import hyperai.config before caffe to set the path
import caffe_pb2  # noqa

logger = logging.getLogger('hyperai.tools.inference')
...
def infer(input_list,
    ...
    model = Job.load(job_id=model_dir)

    # load dataset job
    dataset_dir = os.path.join(config_value('alljobs_dir'), model.dataset_id)
    assert os.path.isdir(dataset_dir), "Dataset dir %s does not exist" % dataset_dir
    dataset = Job.load(path=dataset_dir)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Inference tool - hyperai')
