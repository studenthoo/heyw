...
class LoaderFactory(object):
    ...
    def create_input_pipeline(self):
        ...
        if self.stage == hyperai.STAGE_INF:
            if isinstance(self,TFRecordsLoader):
                single_key, single_data, single_data_shape, _, _ = self.get_single_data_infer(key_queue)
            else:
            # print(type(self))
...
class TFRecordsLoader(LoaderFactory):
    ...
    def get_single_data_infer(self, key_queue):
        """
        Returns:
            key, single_data, single_data_shape, single_label, single_label_shape
        """
