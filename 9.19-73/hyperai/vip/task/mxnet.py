...
class MxnetTrain(TrainTask):
    ...
    def au_process_output(self, line):
        ...
        if data_line:
            if data_line.get('epoch') and data_line.get('batch'):
                ...
                if 'loss' in data_line:
                    self.temp_dict['loss'] = data_line.get('loss')
    ...
    def regular_train_output(self, line, cull=True):
        ...
        line_pattern = re.compile(
            r"(INFO:)(\w+):Epoch\[(\d+)\] Batch \[(\d+)\](\s+)Speed: (\d+.\d+|\d+) samples/sec(\s+)(accuracy|acc)=(\d+.\d+|\d+)\s*((loss)=(\d+.\d+))?")
