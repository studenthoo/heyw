...
class Monitor_Resource:
    ...
    def __init__(self, job):
        self.job = job
        self.data_txt = 'monitor.txt'
        self.res = None
        self.mem = None
        self.mem_total = None

    def hw_socketio_updater(self, pod_name, resources_deployment):
        """
        ...
        while True:
            ...
            with app.app_context():
                ...
                """
                {'#0': {'major': '6', 'temperature': '52', 'utilization': {'gpu': '0%', 'memory': '0%'}, 'memory': {'total': '6072', 'free': '3993', 'used': '2079'}, 'minor': '1', 'name': 'GeForce GTX 1060 6GB'}, 'container_name': 'hy70-20180603-070415-fb32-74d566948b-tnkd8'}
                """
    ...
    def get_gpu_from_pod(self, pod_name_list):
        ...
        for pod in pod_name_list:
            ...
            # print 'file-path', device_query_file, config_value('jobs_dir')
            get_gpu_from_pod = "kubectl exec {} python {}".format(pod, device_query_file)
            # print "train.py [261] : %s" % get_gpu_from_pod
    ...
    def get_data(self):
        with app.app_context():
            ...
            if res:
                for each in res:
                    pod_name = each['container_name']
                    # print each.items()
                    for key, val in each.items():
                        # print key, val
                        if key != 'container_name':
                            if val['utilization']:
                                gpu_count = val['utilization']['gpu']
                                memory = round(float(val['memory']['used']) / float(val['memory']['total']), 2)
                                try:
                                    monitor = Monitor(gpu_utilization=gpu_count, pod_name=pod_name, memory=memory)
