...
def trans_tiff_to_img(ds):

    array = ds.ReadAsArray()

    if array.dtype != np.uint8:
        if array.max() > 255:
            int16_min = hyperai.config.config_value('int16_min')  # 1000
            int16_max = hyperai.config.config_value('int16_max')  # 100
            array = 255.0 * (array - int16_min) / (int16_max - int16_min)
        array = array.clip(0, 255).astype(np.uint8)

    if array.ndim == 2:
        array = np.array([array, array, array])

    if array.shape[0] > 3:
        array = np.array([array[0], array[1], array[2]])
    ...
    try:
        ...
        traceback.print_exc()
        hyperai.log.logger.error(array)

def open_tiff(fpath):
    ds = gdal.Open(fpath)
    return trans_tiff_to_img(ds)

def check_image(image_dir):
    TIF_LIST = ['.tif', '.tiff']
    if os.path.exists(image_dir) and os.path.isdir(image_dir):
        for image in os.listdir(image_dir):
            if os.path.splitext(image)[-1] in TIF_LIST:
                return True
        return False
    return False
