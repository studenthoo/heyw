...
from hyperai.model.tasks import TensorflowTrainTask
from hyperai.model.tasks import TensorflowTrainTaskMpi
...
class TensorflowFramework(Framework):
    ...
    @override
    def create_train_task(self, mpi=False, **kwargs):
        """
        create train task
        """
        if mpi:
            return TensorflowTrainTaskMpi(framework_id=self.framework_id, **kwargs)
        else:
    ...
    def get_standard_network_desc(self, network):
        ...
        """
        networks_dir = os.path.join(os.path.dirname(hyperai.__file__), 'standard-networks', self.CLASS)
    ...
    def get_network_from_previous(self, previous_network, use_same_dataset):
        ...
        """
        # note: use_same_dataset is ignored here because for Tensorflow, hyperai
    ...
    def get_network_visualization(self, **kwargs):
        ...
        try:  # do this in a try..finally clause to make sure we delete the temp file
            # build command line
            args = [sys.executable,
                    os.path.join(os.path.dirname(hyperai.__file__), 'tools', 'tensorflow', 'main.py'),
