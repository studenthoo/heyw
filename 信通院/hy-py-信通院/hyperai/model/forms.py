...
class ModelForm(Form):
    ...
    train_method = utils.forms.SelectField(u'Train method',
                                 choices=[
                                     ('0', 'One'),
                                     ('1', 'More'),
                                 ],
...
#                   raise validators.ValidationError('File "%s" does not exist' % filename)
    ...
    model_desc = utils.forms.StringField(u'model_desc',
    ...
    cpu_count_more = utils.forms.SelectField(
        'Cpu count',
        choices=[
            ('2', '2'),
            ('4', '4'),
    ...
    memory_count_more = utils.forms.SelectField(
        'Memory count',
        choices=[
            ('8', '8G'),
            ('16', '16G'),
    ...
    gpu_count_new = utils.forms.SelectField(
        'gpu_count_new',
        choices=[('%d' % i, '%d' % i) for i in range(1, gpus_max_on_nodes + 1)],
        default='1',
    )
    gpu_count_new_more = utils.forms.SelectField(
        'gpu_count_new',
        choices=[('%d' % i, '%d' % i) for i in range(1, gpus_max_on_nodes+1)],
        default='1',
    )
