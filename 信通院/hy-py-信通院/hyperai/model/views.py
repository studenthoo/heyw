...
from hyperai import frameworks, extensions
from hyperai.utils import auth
from hyperai.utils.routing import request_wants_json, job_from_request, get_request_arg
from hyperai.webapp import scheduler
from hyperai.models import User, Model
...
def to_pretrained(job_id):
    ...
    return flask.redirect(flask.url_for('hyperai.views.home', tab=3)), 302
...
def to_store(job_id):
    ...
    if job.apply_scence != 'other':
        accuracy = str(int(job.train_task().combined_graph_data()['columns'][-3][-1])) + '%'
    else:
        accuracy = '0%'
    model = Model(name=name, publish_user=publish_user,apply_scenes=apply_scence, model_path=model_path, job_id=job_id, framework=framework, network=network,desc=desc, accuracy=accuracy)
    model.permissions = 'S'
    user = User.query.filter_by(name=username).first()
    v = Model.query.filter_by(job_id=job_id).first()
