...
from hyperai.utils.filesystem import tail
# from hyperai.tools.kuber.caffe_pri import CaffePri
...
CAFFE_PYTHON_LAYER_FILE = 'hyperai_python_layers.py'


@subclass
class HyperaiTransformer(caffe.io.Transformer):
    """
...
class CaffeTrainTask(TrainTask):
    ...
    def before_run(self, args=None):
        super(CaffeTrainTask, self).before_run(args=None)

        if isinstance(self.job, hyperai.model.images.classification.ImageClassificationModelJob):
            self.save_files_classification()
        elif isinstance(self.job, hyperai.model.images.generic.GenericImageModelJob):
        ...
        # >>>>  2018-4-26  >>>>
        # create yaml
        caffe_obj = ManagePod(job_id=self.job.id(), logger=self.logger, job_dir=self.job_dir, task_obj=self, output_log=self.log)
        command = ["/usr/bin/python2"]
        args = ['/root/hyperai/hyperai/pid_rand.py',
                '--args=%s' % (" ".join(args)),
                ]
        try:
            new_path = caffe_obj.generate_yaml(gpu_count=self.gpu_count,
                                               cpu_count=self.cpu_count,
                                               memory_count=self.memory_count,
                                               command=command,
                                               args=args,
                                               selector_node="10.18.95.4")
            # <<<<  2018-4-26  <<<<
            self.log.write("----*----Create New_yaml Successed.----*---- \n")
            self.pod_obj = caffe_obj
            self.yaml_path = new_path
            return {'obj': caffe_obj, 'yaml': new_path}
        except Exception as e:
            self.log.write("Created Yaml Failed: %s \n" % e)
            self.logger.error("Created Yaml Failed: %s" % e)
    ...
    def save_files_classification(self):
        ...
        if train_data_layer is not None:
            ...
            if dataset_backend == 'lmdb' and train_data_layer.HasField('data_param'):
                # self.log.write("don't set the data_param.source \n")
                assert not train_data_layer.data_param.HasField('source'), "don't set the data_param.source"
                # self.log.write("don't set the data_param.backend \n")
            ...
            if dataset_backend == 'hdf5' and train_data_layer.HasField('hdf5_data_param'):
                # self.log.write("don't set the hdf5_data_param.source \n")
            ...
            if self.crop_size:
                # self.log.write("HDF5Data layer does not support cropping \n")
                assert dataset_backend != 'hdf5', 'HDF5Data layer does not support cropping'
                # self.log.write('crop_size is larger than the image size \n')
        ...
        if self.use_mean == 'pixel':
            # self.log.write('HDF5Data layer does not support mean subtraction \n')
        ...
        # self.log.write("Network sanity check - deploy \n")
        ...
        for layer in deploy_network.layer:
            if layer.type == 'Softmax':
                found_softmax = True
                break
        # self.log.write('Your deploy network is missing a Softmax layer! '
        ...
        solver.net = self.path(self.train_val_file)
        ...
        solver.snapshot_prefix = self.path(self.snapshot_prefix)

        # Batch accumulation
        from hyperai.frameworks import CaffeFramework
        ...
        if solver.lr_policy == 'fixed':
            ...
            # self.log.write('Unknown lr_policy: "%s" \n' % solver.lr_policy)
    ...
    def task_arguments(self, resources, env):
        ...
        # resources = {'gpus': [('0', 1), ('1', 1), ...]}
        # resources = {'gpus': (resource_obj, self.gpu_count)}
        if 'gpus' in resources:
            resource_obj = resources['gpus'][0]
            value = resources['gpus'][1]
            if value == 1:
                identifiers = ['all']
                args.append('--gpu=%s' % identifiers[0])
            elif value > 1:
                args.append('--gpu=%s' % 'all')
        ...
        if self.memory_count:  # +++
            resources_args['memory_count'] = "%sGi" % self.memory_count  # +++
    ...
    def after_runtime_error(self):
        if os.path.exists(self.path(self.CAFFE_LOG)):
            ...
            if 'HYPERAI_MODE_TEST' in os.environ:
                self.log.write("* * caffe_train.py/after_runtime_error: %s \n"% output)
                print "* * caffe_train.py/after_runtime_error: ", output
        ...
        if hasattr(self, "model_file"):
            if self.model_file is not None:
                stats.update({
                    ...
                    "hyperai version": self.hyperai_version
                })
    ...
    def infer_many_images(self,
        ...
        for chunk in [caffe_images[x:x + data_shape[0]] for x in xrange(0, len(caffe_images), data_shape[0])]:
            ...
            # self.log.write('Processed %s/%s images \n' % (len(outputs[outputs.keys()[0]]), len(caffe_images)))
    ...
    def get_net(self, epoch=None, gpu=None):
        ...
        # Attempt to force a reload of the "hyperai_python_layers" module
        loaded_module = sys.modules.get('hyperai_python_layers', None)
        if loaded_module:
            try:
    ...
    def get_transformer(self, resize=True):
        """
        Returns an instance of HyperaiTransformer
        Parameters:
...
def cleanedUpClassificationNetwork(original_network, num_categories):
    ...
    for i, layer in enumerate(network.layer):
        if 'Data' in layer.type:
            ...
            # hyperai handles the deploy file for you
            del network.layer[i]
