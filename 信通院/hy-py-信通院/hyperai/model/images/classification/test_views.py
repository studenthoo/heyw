...
import hyperai.dataset.images.classification.test_views
...
class UserModel(Tower):
    ...
    @model_property
    def loss(self):
        loss = hyperai.classification_loss(self.inference, self.y)
        return loss
"""
...
class BaseViewsTestWithDataset(BaseViewsTest,
                               hyperai.dataset.images.classification.test_views.BaseViewsTestWithDataset):
    """
    Provides a dataset
    """
...
class TestCaffeLeNet(BaseTestCreated, test_utils.CaffeMixin):
    ...
    CAFFE_NETWORK = open(
        os.path.join(
            os.path.dirname(hyperai.__file__),
...
class TestTensorflowLeNet(BaseTestCreated, test_utils.TensorflowMixin):
    ...
    # standard lenet model will adjust to color
    # or grayscale images
    TENSORFLOW_NETWORK = open(os.path.join(os.path.dirname(hyperai.__file__),
                                           'standard-networks',
                                           'tensorflow',
                                           'lenet.py')).read()
