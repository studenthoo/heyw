...
"""
TensorFlow training executable for hyperai
...
tf.app.flags.DEFINE_float('momentum', '0.9', """Momentum""")  # Not used by hyperai front-end
...
# @TODO(tzaman): is the bitdepth in line with the hyperai team?
...
# Tensorflow-unique arguments for hyperai
tf.app.flags.DEFINE_string(
    'save_vars', 'all',
...
def print_summarylist(tags, vals):
    ...
    that corresponds to how the hyperai regex reads it.
    Args:
    ...
    try:
        while not model.queue_coord.should_stop():
            ...
            for i in range(len(keys)):
                ...
                # We're allowing multiple predictions per image here. hyperai doesnt support that iirc
...
def main(_):
    ...
    if FLAGS.train_db:
        with tf.name_scope(hyperai.STAGE_TRAIN) as stage_scope:
            train_model = Model(hyperai.STAGE_TRAIN, FLAGS.croplen, nclasses, FLAGS.optimization, FLAGS.momentum)
    ...
    if hvd.rank() == 0 and FLAGS.validation_db:
        with tf.name_scope(hyperai.STAGE_VAL) as stage_scope:
            val_model = Model(hyperai.STAGE_VAL, FLAGS.croplen, nclasses, reuse_variable=True)
    ...
    if FLAGS.inference_db:
        with tf.name_scope(hyperai.STAGE_INF) as stage_scope:
            inf_model = Model(hyperai.STAGE_INF, FLAGS.croplen, nclasses)
    ...
    if FLAGS.train_db:
        ...
        # it needs to be rounded to the required number of significant hyperai.
        epoch_round = 0  # holds the required number of significant hyperai for round function.
        tmp_batchsize = batch_size_train * logging_interval_step
        while tmp_batchsize <= train_model.dataloader.get_total():
            tmp_batchsize = tmp_batchsize * 10
            epoch_round += 1
        logging.info("While logging, epoch value will be rounded to %s significant hyperai", epoch_round)
        ...
        try:
            ...
            exit(-1)  # hyperai wants a dirty error.
