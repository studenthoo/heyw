...
class Model(object):
    ...
    creating the model while using the hyperai UI to select the
    ...
    def create_dataloader(self, db_path):
        self.dataloader = tf_data.LoaderFactory.set_source(db_path, is_inference=(self.stage == hyperai.STAGE_INF))
    ...
    def init_dataloader(self):
        with tf.device('/cpu:0'):
            with tf.name_scope(hyperai.GraphKeys.LOADER):
    ...
    def create_model(self, obj_UserModel, stage_scope, batch_x=None):
        ...
        if batch_x is None:
            ...
            if self.stage != hyperai.STAGE_INF:
                batch_y = self.dataloader.batch_y
        else:
            assert self.stage == hyperai.STAGE_INF
            batch_x = batch_x

        available_devices = hyperai.get_available_gpus()
        ...
        if len(available_devices) == 1:
            ...
            if self.stage != hyperai.STAGE_INF:  # Has no labels
        ...
        for dev_i, dev_name in enumerate(available_devices):
            with tf.device(dev_name):
                ...
                with tf.name_scope(current_scope) as scope_tower:
                    ...
                    with tf.variable_scope(hyperai.GraphKeys.MODEL, reuse=dev_i > 0 or self._reuse):
                        ...
                        with tf.name_scope(hyperai.GraphKeys.LOSS):
                            for loss in self.get_tower_losses(tower_model):
                                tf.add_to_collection(hyperai.GraphKeys.LOSSES, loss['loss'])

                            # Assemble all made within this scope so far. The user can add custom
                            # losses to the hyperai.GraphKeys.LOSSES collection
                            losses = tf.get_collection(hyperai.GraphKeys.LOSSES, scope=scope_tower)
                        ...
                        if self.stage == hyperai.STAGE_TRAIN:
                            grad_tower_losses = []
                            for loss in self.get_tower_losses(tower_model):
    ...
    def add_tower(self, obj_tower, x, y):
        is_training = self.stage == hyperai.STAGE_TRAIN
        is_inference = self.stage == hyperai.STAGE_INF
