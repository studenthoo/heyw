...
class CreateGenericDbTask(Task):
    ...
    def __init__(self, job,username, backend, stage, **kwargs):
        """
        Arguments:
        """
        self.job = job
        ...
        self.flag = "ds-create-generic-db"
    ...
    def task_arguments(self, resources, env):
        args = [
            ...
            self.job.id(),
            '--stage=%s' % self.stage,
            '--jobs_dir=%s' % os.path.join(hyperai.config.config_value('jobs_dir'),self.username+'/jobs'),
        ]
        if self.parents:
            args.append("--flag=%s" % self.flag)
        resources_args = {}
