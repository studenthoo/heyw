...
from hyperai.utils.image import check_image
import os
...
class GenericDatasetJob(DatasetJob):
    ...
    def __init__(self,
        ...
        self.apply_scence = extension_id  #+++++++++
        super(GenericDatasetJob, self).__init__(**kwargs)
        self.pickver_job_dataset_extension = PICKLE_VERSION
        self.target_dir = os.path.join(self.dir(), 'dataset')
        # create tasks
        predict_images_task = None
        if feature_folder_path:
            if check_image(feature_folder_path):
                predict_images_task = tasks.ExtractDatasetTask(
                    job_dir=self.dir(),
                    num_threads=self.num_threads,
                    target_dir=self.target_dir,
                    feature_images_path=feature_folder_path,
                    label_images_path=label_folder_path,
                )

                self.tasks.append(predict_images_task)
